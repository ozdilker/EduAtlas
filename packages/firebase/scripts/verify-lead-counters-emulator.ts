/**
 * Verifies `institutions/{institutionId}.leadCounters` is updated by Firestore lead triggers.
 *
 * Intended usage (run with local emulators):
 *   firebase emulators:exec --only firestore,functions --project eduatlas-dev -- npx tsx <this file>
 *
 * It assumes:
 * - An institution with id `seed_inst_ist_kolej_1` exists in Firestore.
 * - `updateLeadCountersOnCreate/Update/Delete` functions are available in the Functions emulator.
 */

import { getAdminFirestore, createFirestoreInstitutionRepository, createFirestoreLeadRepository } from "../src/server/index";
import { createInstitutionId, createLead, createLeadId, LeadRole, LeadStatus, leadIdAsString } from "@eduatlas/domain";
...
const SAMPLE_LEADS: ReadonlyArray<{
  id: string;
  status: LeadStatus;
  email?: string;
...
async function upsertSeedInstitution(firestore: ReturnType<typeof getAdminFirestore>): Promise<void> {
  ...
  await firestore.collection("institutions").doc(id).set(FirestoreInstitutionMapper.toFirestore(institution), { merge: true });
}
...
async function main(): Promise<void> {
  ...
  if (!institution.leadCounters) {
    throw new Error(
      "leadCounters is missing on institution. Expected updateLeadCounters triggers to populate it.",
    );
  }

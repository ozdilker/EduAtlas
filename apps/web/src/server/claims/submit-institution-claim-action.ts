...
/**
 * Server action: institution page claim form → application service → repository.
 */
export async function submitInstitutionClaimAction(
  ...
  try {
    const [institutionRepository, claimRequestRepository, notificationService] = await Promise.all([
    ...
    await submitClaimRequest(
      ...
      { institutionRepository, claimRequestRepository, notificationService },
    );
